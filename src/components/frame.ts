...
export const Frame = Vue.component("refresher-frame", {
    ...
    template: `
      ...
      <div class="refresher-preview-info" v-if="!frame.error">
        <div class="refresher-preview-title-zone">
          ...
          <div v-if="frame.data.comments" class="refresher-comment-controls-container">
            <PreviewButton v-if="frame.data.useWriteComment" :id="'write'" :text="'댓글 쓰기'" :click="toCommentWrite"
      ...
      <div v-if="frame.collapse" class="refresher-collapse-text">
        <h3 v-on:click="() => {frame.collapse = !frame.collapse; frame.functions.load()}">댓글 보기를 클릭하여 댓글만 표시합니다. 여기를 눌러
          글을 볼 수 있습니다.</h3>
      </div>
