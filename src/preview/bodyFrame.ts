...
import {panel} from "../utils/previewPanel";
...
export function makeBodyFrame(ctx: BodyFrameContext): void {
    ...
    let postFetchedData: IPostInfo | undefined;
    ...
    frame.functions.share = async () => {
        await writeClipboard(`https://gall.dcinside.com/${http.galleryType(preData.link!, "/")}board/view/?id=${preData.gallery || http.queryString("id")}&no=${preData.id}`)
