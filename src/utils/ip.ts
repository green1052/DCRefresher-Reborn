...
storage.get<string>("refresher.asn")
    .then((data) => {
        asnReader = new mmdb.Reader<AsnResponse>(Buffer.from(data, "base64"));
    });

storage.get<string>("refresher.country")
    .then((data) => {
...
const CUSTOM_NAME: { [key: string]: string } = {
    ...
    "HANWHA Corp. Information Service div.": "HANWHA Corp. Information Service div.",
    ...
    "HANASK card": "하나SK카드",
    "Seoul Metropolitan Government Computer Center": "Seoul Metropolitan Government Computer Center",
    ...
    // 부정확
    "GYEONGGI PROVINCIAL ANYANG OFFICE OF EDUCATION": "경기도안양과천교육지원청",
